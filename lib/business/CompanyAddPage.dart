...
class CompanyAddPage extends StatefulWidget {
  const CompanyAddPage({Key? key}) : super(key: key);

  @override
  State<CompanyAddPage> createState() => _CompanyAddPageState();
}

class _CompanyAddPageState extends State<CompanyAddPage> with TickerProviderStateMixin {
  ...
  Future<void> _createCompany() async {
    ...
    try {
      ...
      if (_base64Image != null) {
        companyData['image'] = _base64Image!;
      }
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (responseData['status'] == 'success') {
          // Extract the auto-generated company_code from response
          String companyCode = 'N/A';
          if (responseData['data'] != null && responseData['data']['company_code'] != null) {
            companyCode = responseData['data']['company_code'];
          } else if (responseData['message']?.toString().contains('G') == true) {
            companyCode = responseData['message'].toString().split('code: ').last;
          }
          _showSuccessDialog(companyCode);
        } else {
          throw Exception(responseData['message'] ?? 'Unknown error');
        }
      } else if (response.statusCode == 409) {
        // Handle duplicate company name conflict
        final errorData = jsonDecode(response.body);
        throw Exception('Company already exists: ${errorData['details'] ?? errorData['message']}');
  ...
  void _showSuccessDialog(String companyCode) {
    showDialog(
      context: context,
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Company "${_companyNameController.text}" has been created successfully!'),
            SizedBox(height: 8),
            if (companyCode != 'N/A') ...[
        ...
        actions: [
          TextButton(
            onPressed: () {
              ...
              Navigator.of(context).pop(true); // Return to Company list
            },
            child: Text(
              'OK',
              style: TextStyle(
